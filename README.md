...
`s`   | Website name | yes      |
`d`   | Domain       | yes      |
`ad`  | Snap Domain  | no       | Can be used to specify the domain of the results if the bang searches another website. See `hn` bang.
...
`open_snap_domain`         | When the bang is invoked with no query, open the snap domain (`ad`) instead of any path given in the template
